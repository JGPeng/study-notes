...
def set_url():
    ...
    while count <= 82:
        if count <= 15:
            DATAURL.append('https://sx.zxxk.com/m/review-sce3205/area' + str(count) + '0000-' + str(YEAR) + '/')
            count += 1
...
def output_excel():
    for i in range(len(PROVINCE)):
        count = excel_data[PROVINCE[i]][TITLE[i]]
        ws.cell(2, i + 1).value = count
    wb.save(SAVE)
...
def get_page_number(site_data):
    ...
    if len(tree.xpath('//*[@class="list-page"]')) == 0:
        ...
        print(page_data) if len(page_data) > 0 else print('共1页')
        return int(page_data[0][1:-1]) if len(page_data) > 0 else 1
    ...
    if len(div_list) > 0:
        ...
        for j in range(len(list_data)):
            ...
            for index in range(len(PROVINCE)):
                if list_data[j].find(PROVINCE[index]) > -1 and list_data[j].find(KEY) > -1 and list_data[j].find(
                        NOTKEY) == -1:
...
if __name__ == '__main__':
    ...
    for u in range(len(DATAURL)):
        for page in range(1, get_page_number(DATAURL[u]) + 1):
            # 页码为 1 时路由没有“html”的后缀，超过1页时才有
            if page == 1:
                site = DATAURL[u]
            else:
                site = DATAURL[u] + 'index-{n}.html'.format(n=page)
